...
str(cv.10.folds[33])
str(cv.10.folds[[33]]) # nested list, skip a level with [[]]
str(cv.10.folds$Fold03.Rep04)
...
# not necessary to use too many trees
...
# We found the best random forest was rf.5, 
# Now we choose to apply three-fold cross validation to tune our model, 
# making the accuracy close to the test result of the original rf.5
...
rpart.1.cv.1 <- rpart.cv(94622, rpart.train.1, rf.label, ctrl.3) # udf(seed, training, labels, ctrl)
rpart.1.cv.1
# Resampling results across tuning parameters:
#   
#   cp          Accuracy   Kappa    
...
name.splits <- str_split(data.combined$name, ",") # Lastname, Title. Firstname Middlename
name.splits[1]
last.names <- sapply(name.splits, "[", 1) # the first column of name.splits, '[' is index function
`[`(name.splits[[1]], 1) #first element of first sublist 
...
# One female? Mr. converted from Dr. or something
...
preProc <- preProcess(preproc.data.combined, method = c("center", "scale")) # train a preProc Model

postproc.data.combined <- predict(preProc, preproc.data.combined) # normalize data with the preProc Model
...
# Until now we still use cv.3 to train our rpart tree. We updated our features to make our model more accurate.
...
library(Rtsne) # reduce to two dimensions to visualize
...
# output between runs.
set.seed(984357)
tsne.1 <- Rtsne(most.correct[, features], check_duplicates = FALSE) # same four features may represent different people
...
# Run CV and check out results
rpart.4.cv.1 <- rpart.cv(94622, rpart.train.4, rf.label, ctrl.3) # udf(seed, training, labels, ctrl)
rpart.4.cv.1 
prp(rpart.4.cv.1$finalModel, type = 0, extra = 1, under = TRUE)
# Resampling results across tuning parameters:
#   
#   cp          Accuracy   Kappa    
# 0.00000000  0.8153760  0.6053178
# 0.01593063  0.8343434  0.6420371
# 0.03186126  0.8335578  0.6411763
# 0.04779189  0.8248036  0.6250217
# 0.06372252  0.8163861  0.6092666
# 0.07965316  0.8136925  0.6042584
# 0.09558379  0.8121212  0.6012580
# 0.11151442  0.7997755  0.5780290
# 0.12744505  0.7940516  0.5672335
# 0.14337568  0.7934905  0.5662044
# 0.15930631  0.7934905  0.5662044
# 0.17523694  0.7934905  0.5662044
# 0.19116757  0.7934905  0.5662044
...
# Make predictions
rpart.4.preds <- predict(rpart.4.cv.1$finalModel, test.submit.df, type = "class")
table(rpart.4.preds)

# Write out a CSV file for submission to Kaggle
submit.df <- data.frame(PassengerId = rep(892:1309), Survived = rpart.4.preds)

write.csv(submit.df, file = "RPART_SUB_20170104_1.csv", row.names = FALSE)
# 0.80861
...
write.csv(submit.df, file = "RF_SUB_20170104_1.csv", row.names = FALSE)
# 0.78469
